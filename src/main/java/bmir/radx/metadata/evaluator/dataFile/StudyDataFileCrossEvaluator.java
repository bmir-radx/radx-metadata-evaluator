...
//    }
    ...
    if(dataFileStudyPhs!= null){
      compareValues(studyPhs, dataFileName, studyPhs, dataFileStudyPhs, DATA_FILE_PARENT_STUDIES, PHS_IDENTIFIER, inaccurateStudies, validationSummary);
    }

    // Check Study Name
    var dataFileStudyName = InstanceArtifactValueGetter.getStudyName(dataFileArtifact);
    if(dataFileStudyName != null){
      compareValues(studyPhs, dataFileName, studyName, dataFileStudyName, DATA_FILE_PARENT_STUDIES, STUDY_NAME, inaccurateStudies, validationSummary);
    }
  ...
  private void compareValues(String studyPhs,
    ...
    if(!normalizedDataFileMetadata.equals(normalizedStudyMetadata)){
